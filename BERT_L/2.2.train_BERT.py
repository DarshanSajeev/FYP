...
def main():
    # Ensure we use GPU if available for faster training
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Load precomputed tensors (input ids, attention masks, labels)
    ...
    # Accumulate batch-level metrics for plotting/inspection
    graph_data = {
        "batch_loss": [],
        "batch_acc": [],
        "batch_acc_top2": []
    ...
    # Loop over folds: create dataloaders, init model, train and validate
    for fold_i, (train_idx, val_idx) in enumerate(skf.split(all_input_ids, all_labels)):
        print(f"\n================ Fold {fold_i + 1} / 10 ================")

        # Slice tensors for this fold
        ...
        # Initialize a fresh BERT classifier for this fold
        ...
        # Tracking best validation metrics for early stopping
        ...
        for epoch_i in range(EPOCHS):
            ...
            for batch in train_dataloader:
                ...
                # Compute batch-level accuracies (top-1 and top-2)
            ...
            # Concatenate predictions and compute fold-level accuracies
            ...
            if avg_val_loss < best_val_loss:
                ...
                # Save the best model for the first fold as an example
                if fold_i == 0:
                    print("    -> Saving Best Model to 'bert_model_fold_0.pth'")
                    torch.save(model.state_dict(), 'bert_model_fold_0.pth')
            else:
    ...
    with open('../BERT_L/training_graph_data.json', 'w') as f:
