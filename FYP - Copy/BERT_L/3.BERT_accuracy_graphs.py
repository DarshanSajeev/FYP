...
def plot_average_graph():
    filename = '../Graphs/training_graph_data.json'
    ...
    # 2. Reshape the data (Merge the 10 folds)
    # We know you ran 10 folds. We split the long list into 10 separate lists.
    num_folds = 10
    ...
    # 3. Calculate the Average across the 10 folds
    ...
    plt.title("10-Fold Cross-Validation Average Performance", fontsize=14)
    plt.grid(True, which='both', linestyle='-', linewidth=0.5, alpha=0.5)
    plt.legend(loc='center right', frameon=True, facecolor='white', framealpha=1)
    
    plt.tight_layout()
    save_path = os.path.join('..', 'Graphs', 'my_smooth_averaged_graph.png')
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300)
    print("Success! Saved smooth graph as 'my_smooth_averaged_graph.png'")
    plt.show()
